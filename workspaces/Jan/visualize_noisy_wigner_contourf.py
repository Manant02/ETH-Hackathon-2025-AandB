...
def load_and_visualize_states(data_dir):
    """
    Lädt alle Pickle-Dateien aus dem angegebenen Verzeichnis und visualisiert sie.
    """
    ...
    for file in data_path.glob('*.pickle'):
        ...
        try:
            ...
            X, Y = np.meshgrid(state[0], state[1])
            plt.contourf(X, Y, state[2], cmap="seismic", vmin=np.min(state[2]), vmax=np.max(state[2]))
